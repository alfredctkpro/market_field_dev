...
static NSUInteger kCountForNotification = 31;

@implementation ArchiveParser
...
- (void)parseEnded {
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(parser:didParseIndex:)] && [parsedArticless count] > 0) {
        [self.delegate parser:self didParseIndex:self.parsedArticless];
    }
...
static NSString *kName_Set = @"set";
static NSString *kName_Day = @"day";
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *) qualifiedName attributes:(NSDictionary *)attributeDict {
    if ([elementName isEqualToString:kName_Item]) {
        ...
        self.currentArticle.articleId=[attributeDict objectForKey:kName_Id];
        if (![currentString isEqualToString:@""]) {
            self.currentArticle.date=currentString;
        }
             [currentString setString:@""];
        storingCharacters = YES;
    }
    else if([elementName isEqualToString:kName_Set])
    {
        
        storingCharacters = YES;
     [self.currentString appendString: [[NSString alloc]initWithFormat: @"%@\n%@",[attributeDict objectForKey:kName_Date],  [attributeDict objectForKey:kName_Day]] ];
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    if ([elementName isEqualToString:kName_Item]) {
        self.currentArticle.title=currentString ;
         [currentString setString:@""];
